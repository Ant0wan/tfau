...
// getLatestVersionFromGit retrieves the latest version from a Git repository using Git tags.
...
// fetchGitTags fetches all tags from a Git repository.
func fetchGitTags(source string) ([]string, error) {
	// Use the `git ls-remote` command to fetch tags
	cmd := exec.Command("git", "ls-remote", "--tags", source)
	output, err := cmd.Output()
	if err != nil {
		return nil, fmt.Errorf("failed to fetch Git tags: %v", err)
	}
	...
	for _, line := range lines {
		if strings.Contains(line, "refs/tags/") {
			...
			if len(parts) > 1 {
				...
				// Remove any trailing ^{} (Git's way of indicating the commit pointed to by the tag)
